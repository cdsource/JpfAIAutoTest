 ...
 *
 * public class SampleComposite extends Composite { private class ControlAdapterImpl extends
 * ControlAdapter { private Point minimumSize; ControlAdapterImpl(Point size) { this.minimumSize =
 * size; } public void controlResized(ControlEvent e) { Shell shell = (Shell) e.widget; Point size =
 * shell.getSize(); boolean change = false; if (size.x < this.minimumSize.x) { size.x =
 * this.minimumSize.x; change = true; } if (size.y < this.minimumSize.y) { size.y =
 * this.minimumSize.y; change = true; } if (change) shell.setSize(size); } } public
 * SampleComposite(Composite parent, int style) { super(parent, style); GridLayout gridLayout = new
 * GridLayout(); setLayout(gridLayout); Label label = new Label(this, SWT.NONE);
 * label.setText("Press the button to close:"); label.setLayoutData(new
 * GridData(GridData.HORIZONTAL_ALIGN_CENTER)); Button button = new Button(this, SWT.PUSH);
 * button.setText("OK"); button.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_CENTER));
 * button.addSelectionListener(new SelectionAdapter() { public void widgetSelected(SelectionEvent e)
 * { Shell shell = ((Button) e.widget).getShell(); shell.close(); } }); } public static void
 * main(String[] args) { try { Display display = new Display(); Shell shell = new Shell(display);
 * shell.setText("Sample Composite"); shell.setLayout(new FillLayout()); SampleComposite
 * sampleComposite = new SampleComposite(shell, SWT.NONE); shell.pack();
 * shell.addControlListener(sampleComposite.new ControlAdapterImpl(shell.getSize())); shell.open();
 * while (!shell.isDisposed()) { if (!display.readAndDispatch()) display.sleep(); }
 * display.dispose(); } catch (Exception e) { e.printStackTrace(); } } }
 * 
 * You should run the main method inside the Eclipse IDE. Do not forget to add the jvm option
 * java.library.path specifing the SWT native library. On a Windows system it may look like this:
...
public class ASTMain {
  ...
  static private final String[] IMPORTS = {"org.eclipse.swt.SWT", "org.eclipse.swt.events.*",
  ...
  private void compileAndRun(ICompilationUnit unit) {
    ...
    Compiler compiler = new Compiler(new NameEnvironmentImpl(unit),
        DefaultErrorHandlingPolicies.proceedWithAllProblems(), settings, requestor,
    ...
    if (!error) {
      try {
        ClassLoader loader =
            new CustomClassLoader(getClass().getClassLoader(), requestor.getResults());
