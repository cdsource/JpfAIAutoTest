...
public class JpfUtInfo {
	...
	private void initMethodSetup() {
		...
		sb.append("  *  ").append("\n");
		sb.append("  * @generatedBy wupf@aliyun.com at ").append(AiDateTimeUtil.getCurrDateTime()).append("\n");
		sb.append("  */  ").append("\n");
	...
	public String findImport(final String strClassSimpleName) {
		...
		while (it.hasNext()) {
			...
			if (entry.getKey().endsWith(".*")) {
				String strClassName = entry.getKey().substring(0, entry.getKey().length() - 1) + strClassSimpleName;
