...
public class Stemmer {
	...
	public static void step11(String strKey) {
	...
	public static void step12(String strKey) {
	...
	public static void addVector(Vector<String> v,String strKey,int i,int j)
	{
		if (j<i)
		{
			//logger.debug(strKey.substring(j, i));
			v.add(strKey.substring(j, i));
			//logger.debug(v.size());
		}
	}
	/**
	 * @category 根据大写来判断，驼峰原则
	 * @param strKey
	 */
	public static void step13(String strKey) {
		logger.debug(strKey);
		char[] ca = strKey.toCharArray();
		Vector<String> v = new Vector<String>();
		int j = 0;
		logger.debug(ca.length);
		for (int i = 0; i < ca.length; i++) {
			
			if (Character.isUpperCase(ca[i])) {
				addVector(v,strKey,i,j);
				j = i;
		...
		addVector(v,strKey,strKey.length(),j);
		logger.debug(v.size());
		for (int m = 0; m < v.size(); m++) {
			System.out.println(v.get(m));
		}
	}
	...
	public static void main(String[] args) {
		try {
			...
			String strKey = "phoneNum";
			strKey = "update_toAwardState-5_6aDe";
			// String strKey="str_customer_Name";
			step13(strKey);

			strKey = "ACCOUNTPAGEQUERYLOGID";
			// String strKey="str_customer_Name";
			step13(strKey);
