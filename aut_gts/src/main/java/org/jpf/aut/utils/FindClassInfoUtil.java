    ...
    private void init() throws Exception {
        ...
        for (int i = 0; i < vJavaFilesAll.size(); i++) {
            if (vJavaFilesAll.get(i).indexOf("target") == -1 || vJavaFilesAll.get(i).indexOf("classes") == -1) {
                int iPos=vJavaFilesAll.get(i).indexOf(AiTestConst.MAIN_SRC);
                String strFullClassName=vJavaFilesAll.get(i).substring(iPos+AiTestConst.MAIN_SRC.length()+1, vJavaFilesAll.get(i).length()-FILE_TYPE_JAVA.length());
