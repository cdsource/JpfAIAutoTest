...
public class GenerateUtil2 {
	 ...
	 */
	/*
	 * public static ParamInitBody[] addMethodParamInit(List MethodParam, JpfUtInfo
	 * cJpfUtInfo) {
	 * 
	 * ParamInitBody[] cParamInitBodys = new ParamInitBody[MethodParam.size()];
	 * StringBuffer sb = new StringBuffer(); for (int i = 0; i < MethodParam.size();
	 * i++) { // System.out.println(MethodParam.get(i)); cParamInitBodys[i] = new
	 * ParamInitBody(); // formatToParamBody(cParamInitBodys[i],
	 * MethodParam.get(i).toString()); if (cParamInitBodys[i].isArray()) { // 数组
	 * addArrayParamInit(cParamInitBodys[i], cJpfUtInfo); } else if
	 * (cParamInitBodys[i].getParamType().equalsIgnoreCase("int") ||
	 ...
	 * cParamInitBodys[i].
	 * setParamPreInit("    Class.forName(\"com.mysql.jdbc.Driver\");");
	 * cParamInitBodys[i].
	 ...
	 * sb.append(addListInit(MethodParam.get(i).toString(),
	 * cParamInitBodys[i].getParamType())); } else if
	 * (cParamInitBodys[i].getParamType().startsWith("Integer")) {
	 * sb.append(addIntegerInit(MethodParam.get(i).toString(),
	 * cParamInitBodys[i].getParamType())); } else if
	 * (cParamInitBodys[i].getParamType().startsWith("Map")) {
	 * sb.append(addMapInit(MethodParam.get(i).toString(),
	 * cParamInitBodys[i].getParamType()));
	 * 
	 * } else if (cParamInitBodys[i].getParamType().startsWith("Long")) { //// Long
	 * sysParameterID =1L; sb.append(addLongInit(MethodParam.get(i).toString(),
	 * cParamInitBodys[i].getParamType())); } else if
	 ...
	 * sb.append(addFileInit(MethodParam.get(i).toString(),
	 * cParamInitBodys[i].getParamType())); } else if
	 * (cParamInitBodys[i].getParamType().equalsIgnoreCase("Element")) {
	 * sb.append("    ").append(MethodParam.get(i)).
	 * append(" = EasyMock.createMock(Element.class); " ); } else if
	 * (cParamInitBodys[i].getParamType().equalsIgnoreCase("HttpMethod")) {
	 * sb.append("    ").append(MethodParam.get(i)).
	 * append(" = EasyMock.createMock(HttpMethod.class); "); } else {
	 * sb.append("    ").append(MethodParam.get(i)).append(" =new ");
	 * sb.append(cParamInitBodys[i].getParamType()).append("();"); } // HttpMethod
	...
	public static StringBuilder addMethodParam2Method(int Modifiers, List MethodParam, JpfUtInfo cJpfUtInfo) {
		...
		for (int i = 0; i < MethodParam.size(); i++) {
			 ...
			 * strParamName.indexOf("["); if (iPos > 0) { strParamName =
			 * strParamName.substring(0, iPos); }
			 */
	...
	public static ArrayList<String> addMethodParamInit2(List cMethodParam, JpfUtInfo cJpfUtInfo, int MaxCond) {
		...
		for (int i = 0; i < cMethodParam.size(); i++) {
			...
			if (cMethodParam.get(i).toString().indexOf("<T") > 0 || cMethodParam.get(i).toString().indexOf("<?") > 0) {
			...
			case "Class<T>":
				 ...
				 * this.getClass().getGenericSuperclass();// 获取当前new的对象的泛型的父类的类型 this.clazz =
				 * (Class<T>) ptype.getActualTypeArguments()[0]; // 获取第一个类型参数的真实类型model =
