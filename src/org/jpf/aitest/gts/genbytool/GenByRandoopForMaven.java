 ...
 * 
 */
public class GenByRandoopForMaven {
  ...
  <scope>test</scope>
	 ...
	 * 
	 */
	public GenByRandoopForMaven(String strPrjPath) {
	...
	private void doWork(String strPrjPath) {
		...
		try {
			...
			String strCmd="cmd /c "+strPrjPath.substring(0,2) +" && cd "+strPrjPath+"  &&  mvn -DmemoryInMB=2000 -Dcores=4 evosuite:generate evosuite:export && mvn evosuite:info";
			runexec(strCmd);
			
			new GenByEvoClean(strPrjPath);
			//new UtRunCheck(strPrjPath);
			
			AiFileUtil.delFile(strPrjPath+"\\pom.xml");
			AiFileUtil.Rename(strPrjPath+"\\pom2.xml", strPrjPath+"\\pom.xml");
		}catch(Exception ex)
		{
			logger.error(ex);
			ex.printStackTrace();
	...
	public static void main(String[] args) {
		...
		if (1==args.length)
		{
			new GenByRandoopForMaven(args[0]);
		}else
		{
