...
* @version 创建时间：2016年5月16日 上午9:42:56 
* 类说明 
*/
...
public class UnitTestTotal {
            ...
            if (f1.isDirectory()) {
                ...
                for (int i = 0; i < f2.length; i++) {
                    if (f2[i].isDirectory() && f2[i].getPath().endsWith("_sonar")) {
                        ...
                        doWork(AiFileUtil.getFileName(f2[i].getPath()), f2[i].getPath() + "/builds/" + (j - 1) + "/log",
                                j - 1);
                    }
                }
    ...
    private void insertDb(Connection conn, String strJobName, int iNumber, String str1, String str2) {
        ...
        try {
            ...
            String strSql = "insert into unittesttotal(jobname,testname,tests,time_elapsed,failures,errors,skipped,build_number) values("
                    + "'" + strJobName + "'"
                    + ",'" + cUnitTestInfo.getTestName() + "'"
                    + "," + cUnitTestInfo.getTests()
                    + "," + cUnitTestInfo.getTimeElapsed()
                    + "," + cUnitTestInfo.getFailures()
                    + "," + cUnitTestInfo.getErrors()
                    + "," + cUnitTestInfo.getSkips()
                    + "," + iNumber
                    + ")";
            logger.debug(strSql);
