...
public class GenerateUtil {
     ...
     * update 2018年2月4日
     */
    public static String findStrictClassPath(String strClassName,JpfUtInfo cJpfUtInfo)
    {
        //classname
        String[] strImports=cJpfUtInfo.getUtImport().split("\n");
        for(int i=0;i<strImports.length;i++)
        {
            if (strImports[i].trim().endsWith(strClassName+";"))
            {
                return strImports[i];
            }
        }
...
*/
    ...
    public static String addMethodJavaDoc(int Modifiers, String StrReturn, String strMethodName, List MethodParam,
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("\n").append("  /**").append("\n")
        .append("  * Run the ").append(strMethodName).append(" method test.").append("\n")
        .append("  *").append("\n")
