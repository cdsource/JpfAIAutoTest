...
public class ParamValueFromSelfLog {
    ...
    public static void main(String[] args) {
        try {
            ParamValueFromSelfLog.getInstance().findValueFromLog("com.asiainfo.ebiz.alipay.service.AlipayAccountManager.queryAccountLog");
    ...
    private void init() throws Exception {
        ...
        AiFileUtil.getFiles(GenerateInputParam.SELF_LOG_FILEPATH, vXmlFiles, ".log");
        for (int j = 0; j < vXmlFiles.size(); j++) {
            ...
            try {
                ...
                while ((line = reader.readLine()) != null) {
                    // datetime
                    int iPos = line.trim().indexOf(HandleLogConst.KEY_SelfTest_Method);
                    if (iPos >= 0) {
                        strMethodName = line
                                .substring(iPos + HandleLogConst.KEY_SelfTest_Method.length(), line.length()).trim();
